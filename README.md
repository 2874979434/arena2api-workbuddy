# Arena2API+workbuddy
在原作者基础上修改的sever.py可以连接workbuddy的自定义模型进行对话，不建议暴力使用，个人使用即可
...
本项目仅供学习、研究和技术交流，禁止用于任何违反法律法规、侵犯他人权益或违反第三方服务条款的用途。

使用者在使用本项目时，必须：
1. 遵守所在国家/地区法律法规；
2. 遵守 arena.ai、GitHub、OpenAI 及相关第三方平台的服务条款；
3. 不得将本项目用于未经授权访问、绕过安全/风控/付费机制、批量滥用、商业倒卖、网络攻击、诈骗等非法或不当活动。

本项目按“现状”提供，不提供任何明示或默示担保。作者不对使用者的任何行为负责，也不对因使用本项目产生的任何直接或间接损失、法律责任、纠纷或后果承担任何责任。任何非法、违规或侵权活动均与作者无关，由使用者自行承担全部责任。

本项目与 arena.ai、OpenAI、Anthropic、Google 等公司无任何关联，也不代表其认可。相关商标和服务归各自所有者。

若本项目无意侵犯了您的权益，请联系删除。继续使用即表示您已阅读并同意本声明。
